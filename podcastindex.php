...
//Get v4v info from lehmancreations
// Pulls the info from lehmancreations and saves them in a transient
function getv4vInfo() {
	 // Do we have this information in our transients already?
  $transient = get_transient( 'v4vInfo' );
  ...
  // Nope!  We gotta make a call.
    ...
    $url = 'https://lehmancreations.com/wp-json/v4v/v1/info';
	  
	  
   $args = array(
     'headers' => array(
        'User-Agent' => 'PodcastNamespacePlugin/1.0'
      ),
    );
    ...
    set_transient('v4vInfo', $body, 86400 );
    
    // Return the info.  The function will return here the first time it is run, and then once again, each time the transient expires.
    return $body;
...
class pc20liveItemTag {
	private $pc20live_item_tag__options;

	public function __construct() {
		add_action( 'admin_menu', array( $this, 'pc20live_item_tag__add_plugin_page' ) );
		add_action( 'admin_init', array( $this, 'pc20live_item_tag__page_init' ) );
	}

	public function pc20live_item_tag__add_plugin_page() {
		add_management_page(
			'Live Item Tag 🔥', // page_title
			'Live Item Tag 🔥', // menu_title
			'manage_options', // capability
			'pc20live-item-tag', // menu_slug
			array( $this, 'pc20live_item_tag__create_admin_page' ) // function
		);
	}

	public function pc20live_item_tag__create_admin_page() {
		$this->pc20live_item_tag__options = get_option( 'pc20live_item_tag__option_name' ); ?>
		...
		<div class="wrap">
			...
			<form method="post" action="options.php">
				<?php
					settings_fields( 'pc20live_item_tag__option_group' );
					do_settings_sections( 'pc20live-item-tag-admin' );
					submit_button();
				?>
			</form>
		</div>
	...
	public function pc20live_item_tag__page_init() {
		register_setting(
			'pc20live_item_tag__option_group', // option_group
			'pc20live_item_tag__option_name', // option_name
			array( $this, 'pc20live_item_tag__sanitize' ) // sanitize_callback
		);

		add_settings_section(
			'pc20live_item_tag__setting_section', // id
			'Settings', // title
			array( $this, 'pc20live_item_tag__section_info' ), // callback
			'pc20live-item-tag-admin' // page
		);
		...
		add_settings_field(
			...
			'pc20live-item-tag-admin', // page
			'pc20live_item_tag__setting_section' // section
		);
		...
		add_settings_field(
			'startdate_2', // id
			'StartDate', // title
			array( $this, 'startdate_2_callback' ), // callback
			'pc20live-item-tag-admin', // page
			'pc20live_item_tag__setting_section' // section
		);
		...
		add_settings_field(
			'enddate_3', // id
			'EndDate', // title
			array( $this, 'enddate_3_callback' ), // callback
			'pc20live-item-tag-admin', // page
			'pc20live_item_tag__setting_section' // section
		);
		...
		add_settings_field(
			'title_4', // id
			'Title', // title
			array( $this, 'title_4_callback' ), // callback
			'pc20live-item-tag-admin', // page
			'pc20live_item_tag__setting_section' // section
		);
		...
		add_settings_field(
			'description_5', // id
			'Description', // title
			array( $this, 'description_5_callback' ), // callback
			'pc20live-item-tag-admin', // page
			'pc20live_item_tag__setting_section' // section
		);
		...
		add_settings_field(
			'guid_6', // id
			'GUID', // title
			array( $this, 'guid_6_callback' ), // callback
			'pc20live-item-tag-admin', // page
			'pc20live_item_tag__setting_section' // section
		);
		...
		add_settings_field(
			'enclousure_7', // id
			'Enclousure', // title
			array( $this, 'enclousure_7_callback' ), // callback
			'pc20live-item-tag-admin', // page
			'pc20live_item_tag__setting_section' // section
		);
		...
		add_settings_field(
			'contentlink_8', // id
			'contentLink', // title
			array( $this, 'contentlink_8_callback' ), // callback
			'pc20live-item-tag-admin', // page
			'pc20live_item_tag__setting_section' // section
		);
		...
		add_settings_field(
			'link_9', // id
			'Link', // title
			array( $this, 'link_9_callback' ), // callback
			'pc20live-item-tag-admin', // page
			'pc20live_item_tag__setting_section' // section
		);
		...
		add_settings_field(
			'image_10', // id
			'Coverart', // title
			array( $this, 'image_10_callback' ), // callback
			'pc20live-item-tag-admin', // page
			'pc20live_item_tag__setting_section' // section
		);
	}

	public function pc20live_item_tag__sanitize($input) {
		$sanitary_values = array();
		...
		if ( isset( $input['startdate_2'] ) ) {
			$sanitary_values['startdate_2'] = sanitize_text_field( $input['startdate_2'] );
		}

		if ( isset( $input['enddate_3'] ) ) {
			$sanitary_values['enddate_3'] = sanitize_text_field( $input['enddate_3'] );
		}

		if ( isset( $input['title_4'] ) ) {
			$sanitary_values['title_4'] = sanitize_text_field( $input['title_4'] );
		}

		if ( isset( $input['description_5'] ) ) {
		//	$sanitary_values['description_5'] = esc_textarea( $input['description_5'] );
		$sanitary_values['description_5'] = $input['description_5'];
		}

		if ( isset( $input['guid_6'] ) ) {
			
			if ( isset( $_POST['generate_Lit_UUID'] ) ) {
				$request = wp_remote_get( 'https://www.uuidgenerator.net/api/guid' );
				$body = wp_remote_retrieve_body( $request );
				$sanitary_values['guid_6'] = sanitize_text_field( $body );
		
			} else {
				$sanitary_values['guid_6'] = sanitize_text_field( $input['guid_6'] );
		
			}
		}
		if ( isset( $input['enclousure_7'] ) ) {
			$sanitary_values['enclousure_7'] = sanitize_text_field( $input['enclousure_7'] );
		}

		if ( isset( $input['contentlink_8'] ) ) {
			$sanitary_values['contentlink_8'] = sanitize_text_field( $input['contentlink_8'] );
		}

		if ( isset( $input['link_9'] ) ) {
			$sanitary_values['link_9'] = sanitize_text_field( $input['link_9'] );
		}
		
		if ( isset( $input['image_10'] ) ) {
			$sanitary_values['image_10'] = sanitize_text_field( $input['image_10'] );
		}
	...
	public function pc20live_item_tag__section_info() {
		
	}

	public function enabled_0_callback() {
		printf(
			'<input type="checkbox" name="pc20live_item_tag__option_name[enabled_0]" id="enabled_0" value="enabled_0" %s> <label for="enabled_0">Put the item in your feed</label>',
			( isset( $this->pc20live_item_tag__options['enabled_0'] ) && $this->pc20live_item_tag__options['enabled_0'] === 'enabled_0' ) ? 'checked' : ''
		);
	}
	...
	public function title_4_callback() {
		printf(
			'<input class="regular-text" type="text" name="pc20live_item_tag__option_name[title_4]" id="title_4" value="%s">',
			isset( $this->pc20live_item_tag__options['title_4'] ) ? esc_attr( $this->pc20live_item_tag__options['title_4']) : ''
		);
	}

	public function description_5_callback() {
		printf(
			'<textarea class="large-text" rows="5" name="pc20live_item_tag__option_name[description_5]" id="description_5">%s</textarea>',
			isset( $this->pc20live_item_tag__options['description_5'] ) ? esc_attr( $this->pc20live_item_tag__options['description_5']) : ''
		);
	}

	public function guid_6_callback() {
		printf(
			'<input class="regular-text" type="text" name="pc20live_item_tag__option_name[guid_6]" id="guid_6" value="%s">' ,
...
<?php
	...
	public function enclousure_7_callback() {
		printf(
...
if ( is_admin() )
	$pc20live_item_tag_ = new pc20liveItemTag();

/* 
 * Retrieve this value with:
 * $pc20live_item_tag__options = get_option( 'pc20live_item_tag__option_name' ); // Array of All Options
 * $enabled_0 = $pc20live_item_tag__options['enabled_0']; // Enabled
 * $status_1 = $pc20live_item_tag__options['status_1']; // Status
 * $startdate_2 = $pc20live_item_tag__options['startdate_2']; // StartDate
 * $enddate_3 = $pc20live_item_tag__options['enddate_3']; // EndDate
 * $title_4 = $pc20live_item_tag__options['title_4']; // Title
 * $description_5 = $pc20live_item_tag__options['description_5']; // Description
 * $guid_6 = $pc20live_item_tag__options['guid_6']; // GUID
 * $enclousure_7 = $pc20live_item_tag__options['enclousure_7']; // Enclousure
 * $contentlink_8 = $pc20live_item_tag__options['contentlink_8']; // contentLink
 * $link_9 = $pc20live_item_tag__options['link_9']; // Link
 * $image_10 = $pc20live_item_tag__options['image_10']; // ImageURL
 */

  

function PC20LIT_rss2_head() {
  $pc20live_item_tag__options = get_option( 'pc20live_item_tag__option_name' ); // Array of All Option
  $enabled_0 = $pc20live_item_tag__options['enabled_0']; // Enabled
  $status_1 = $pc20live_item_tag__options['status_1']; // Status
  $startdate_2 = $pc20live_item_tag__options['startdate_2']; // StartDate
  $enddate_3 = $pc20live_item_tag__options['enddate_3']; // EndDate
  ...
  $image_10 = $pc20live_item_tag__options['image_10']; // ImageURL
	...
	global $powerpress_feed;
		...
		<link>'.$link_9.'</link>';
			...
			if ($image_10) {
				echo '
		<itunes:image href="' . $image_10 . '"/>';
			} 

		
	echo PHP_EOL .'</podcast:liveItem>'. PHP_EOL;	
		}
	}


 add_action('rss2_head', 'PC20LIT_rss2_head',30);
